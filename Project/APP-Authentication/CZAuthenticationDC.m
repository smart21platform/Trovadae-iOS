     ...
     [[NSArray alloc] initWithObjects:@"public_profile", @"email", @"user_friends", nil]
                                  ...
                                  completionHandler:^(FBSession *session, FBSessionState state, NSError *error) {
                                      if (error) {
                                          ...
                                          [self.delegate didRegister:errorMessage];
                                      } else {
                                          NSLog(@"User now has publish permissions!");
...
- (void)sessionStateChanged:(FBSession *)session state:(FBSessionState) state error:(NSError *)error
{
    ...
    if (error) {
        ...
        [self.delegate didRegister:errorFacebook];
...
-(void)sendRegistrationWithFormData:(UIImage *)imageProfile textNameComplete:(NSString *)textNameComplete textUsername:(NSString*)textUsername textNumberPhone:(NSString*)textNumberPhone textEmail:(NSString*)textEmail textPassword:(NSString*)textPassword{
    ...
    NSData *jsonPropertiesData = [[NSData alloc] init];
    if(textNumberPhone && textNumberPhone.length>0){
        NSArray *valuesPhone = @[textNumberPhone];
        NSMutableDictionary *dict = [NSMutableDictionary dictionary];
        [dict setValue:@"numberPhone" forKey:@"_id"];
        [dict setValue:@"numberPhone" forKey:@"displayName"];
        [dict setValue:valuesPhone forKey:@"values"];
        NSDictionary *finalData = [NSDictionary dictionaryWithObject:dict forKey:@"numberPhone"];
        NSDictionary *popertiesData = [NSDictionary dictionaryWithObject:finalData forKey:@"properties"];
        
        NSLog(@"\n---------- popertiesData %@",popertiesData);
        jsonPropertiesData = [NSJSONSerialization dataWithJSONObject:popertiesData options:NSJSONWritingPrettyPrinted error:nil];
    ...
    //[postData appendData:[boundaryString dataUsingEncoding:NSUTF8StringEncoding]];
    //[postData appendData:jsonPropertiesData];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (statusCode < 400) {
        ...
        if (statusCode == ERROR_HTTP_USERNAME_USED) {
            msg = NSLocalizedString(@"UsernameAlreadyUsedLKey", nil);
            ...
            msg = NSLocalizedString(@"EmailAlreadyUsedLKey", nil);
        } else if (statusCode == ERROR_HTTP_USERNAME_INVALID) {
            //NSString *title = NSLocalizedString(@"RegistrationErrorTitleLKey", nil);
            msg = NSLocalizedString(@"UsernameInvalidLKey", nil);
            //self.imageUsername.image = [UIImage imageNamed:@"real_name_red"];
        } else {
            NSLog(@"Unknown error!");
            //NSString *title = NSLocalizedString(@"RegistrationErrorTitleLKey", nil);
            msg = NSLocalizedString(@"UnknownRegistrationErrorLKey", nil);
