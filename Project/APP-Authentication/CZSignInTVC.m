...
- (void)textFieldDidBeginEditing:(UITextField *)textField{
    ...
    if(self.textTelephone.text==0){
        self.imageTelephone.image = [UIImage imageNamed:@"ico_poi_telefono"];
    }
}
...
//    }
    ...
    [DC animationAlpha:self.imageBackground];
    [DC saveImageWithoutDelegate:userImage nameImage:NAME_IMAGE_PROFILE key:KEY_IMAGE_PROFILE];
}
...
//                    [self performSegueWithIdentifier:@"unwindToAuthenticationVC" sender:self];
//                }else{
//                    errorMessage = [NSString stringWithFormat:NSLocalizedStringFromTable(@"ErrorSaveCompleteProfile", @"CZ-AuthenticationLocalizable", @"")];
//                    [self animationMessageError:errorMessage];
//                }
//            }];
        } else {
...
-(void)registrationUser {
    ...
    NSString *emailValue = [self.textEmail.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    NSString *passwordValue = [self.textPassword.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    NSString *numberPhone = [self.textTelephone.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
...
-(void)singInViewController{
    NSLog(@"*************** singInViewController: %@ ***************", self.textEmail.text);
    if([self validateForm]){
        ...
        //DC.fbNumberPhone = self.textTelephone.text;
        [DC sendRegistrationWithFormData:self.imageProfile.image textNameComplete:self.textNameComplete.text textUsername:self.textUsername.text textNumberPhone:self.textTelephone.text textEmail:self.textEmail.text textPassword:self.textPassword.text];
    }
}

-(void)didRegister:(NSString *)msg {
    NSLog(@"Registrazione ok");
    [self hideWaiting];
    if (!msg) {
        NSLog(@"not error");
        //updateUserDC.delegate = self;
        SHPUpdateUserCustomPropertiesDC *updateUserDC;
        updateUserDC = [[SHPUpdateUserCustomPropertiesDC alloc] init];
        [self.applicationContext.connections setObject:updateUserDC forKey:@"updatingPropertiesDC"];
        updateUserDC.properties = [NSDictionary dictionaryWithObjectsAndKeys:self.textTelephone.text,@"numberPhone" ,nil];
        self.applicationContext.loggedUser.email = self.textEmail.text;
        self.applicationContext.loggedUser.numberPhone = self.textTelephone.text;
        
        [updateUserDC updateCommand:self.applicationContext.loggedUser];
        [self dismissViewControllerAnimated];
    } else {
        NSLog(@"error during registration");
