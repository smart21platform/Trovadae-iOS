...
- (void)signout {
    [self unregisterDeviceAndUserFromProvider];
    [SHPAuth deleteLoggedUser];
    self.loggedUser = nil;
    [FBSession.activeSession closeAndClearTokenInformation];
    //[self switchToSignedTabs:NO];
}
...
-(void)signin:(SHPUser *)user {
    ...
    NSLog(@"\n %@ fullName", self.loggedUser.fullName);
    NSLog(@"\n %@ httpBase64Auth in.", self.loggedUser.httpBase64Auth);
...
+(void)openConversationViewWith:(NSString *)userid context:(SHPApplicationContext *)context {
    ...
    NSDictionary *tabBarDictionary = [context.plistDictionary objectForKey:@"BarTab"];
    ...
    if (messages_tab_index >= 0) {
        ...
        conversationsVC.selectedRecipient = userid;
