//
//  SHPShareSettingsViewController.m
//  Ciaotrip
//
...
@interface SHPShareSettingsViewController ()

@end

@implementation SHPShareSettingsViewController
...
- (void)viewDidLoad
{
    ...
    // localize labels
    self.currentFacebookAccountLabel.text = NSLocalizedString(@"ShareOnLKey", nil);
}
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"selectFacebookAccountSegue2"]) {
...
- (IBAction)selectFbPage:(id)sender {
    [self performSegueWithIdentifier:@"selectFacebookAccountSegue2" sender:self];
}
